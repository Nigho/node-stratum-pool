...
var bitcore = require('bitcore-lib-mac')
    ...
    if (rpcData.masternode && rpcData.superblock) {
        if (rpcData.masternode.length > 0) {
            for(var i in rpcData.masternode) {
                ...
                var payeeAddress = bitcore.Address.fromString(rpcData.masternode[i].payee)
                var payeeScript = bitcore.Script.buildPublicKeyHashOut(payeeAddress).toBuffer()

                console.log(payeeAddress.toString())
